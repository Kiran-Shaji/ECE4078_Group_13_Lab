...
class RRT:
    ...
    def get_random_node(self):
        x = self.width * np.random.random_sample() - self.width/2
        y = self.height * np.random.random_sample()- self.height/2
        rnd = self.Node(x, y)
        return rnd
...
# Example usage:
file_path = 'M4_prac_map_full.txt'  # Replace with your actual file path
map_data = load_map(file_path)

# Now, `map_data` is a dictionary containing the data from the file
print(map_data)

shopping = open("M4_prac_shopping_list.txt", "r")
data = shopping.read()
...
for fruit in shopping_list:
    ...
    for name, coord_data in map_data.items():
        if name.startswith(current_fruit):
            ...
            obstacles.append((coord_data['x'], coord_data['y']))
    ...
    rrt = RRT(start=start, goal=goal, width=16, height=10, obstacle_list=obstacle_circles,
